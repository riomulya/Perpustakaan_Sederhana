...
$get_data = read("SELECT * FROM tbl_anggota WHERE id_anggota = '$get_id'")[0];
...
if (isset($_POST['submit'])) {
    ...
    $alamat = $_POST["alamat"];
    //update
    $update = query("UPDATE tbl_anggota SET nama = '$nama',jenis_kelamin = '$jenis_kelamin',alamat = '$alamat' WHERE id_anggota = '$get_id'");
                ...
                <form class="border border-secondary" method="post">
                    <table>
                        <tr>
                            ...
                            <td>
                                <input type="text" name="id_anggota" id="inputID" class="input-group-text" value="<?= $get_data['id_anggota']; ?>" readonly>
                            </td>
                        </tr>
                        <tr>
                            <td><label for="inputNama" class="mt-5">Nama </label></td>
                            <td><input type="text" name="nama" id="inputNama" class="input-group-text mt-5" value="<?= $get_data['nama']; ?>" required>
                            </td>
                        </tr>
                        <tr>
                            <td><label class="mt-5">Jenis Kelamin </label></td>
                            <td>
                                <?php if ($get_data['jenis_kelamin'] === "Wanita") { ?>
                                    ...
                                    <span class="form-check form-check-inline">
                                        <input class="form-check-input mt-2 ms-3" type="radio" id="Wanita" value="Wanita" name="jenis_kelamin" checked>
                                        <label class="form-check-label mt-2 ms-1" for="Wanita">
                                            Wanita
                                        </label>
                                    </span>

                                <?php } elseif ($get_data['jenis_kelamin'] === "Pria") { ?>
                                    <span class="form-check form-check-inline">
                                        <input class="form-check-input mt-5 ms-3" type="radio" id="pria" value="Pria" name="jenis_kelamin" checked>
                                        <label class="form-check-label mt-5 ms-1" for="pria">
                                            Pria
                                        </label>
                                    </span>
                        ...
                        <tr>
                            ...
                            <td>
                                <textarea class="form-control mt-5" id="inputAlamat" rows="3" name="alamat"><?= $get_data['alamat']; ?></textarea>
                            </td>
                        </tr>
                    </table>
